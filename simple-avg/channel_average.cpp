...
static ModelError_t addNode__ReduceMean(QnnModel& model){
  ...
  uint32_t dimensions___ReduceMean_axes[] = {1};
  uint32_t __ReduceMean_axes[] = {3};
     ...
     {.tensorParam=(Qnn_Tensor_t) {
          ...
          {.v2= {
            ...
            .quantizeParams= { QNN_DEFINITION_UNDEFINED,
                               ...
                               {.scaleOffsetEncoding= {.scale= 0.0000000000000000f, .offset= 0}}},
            .rank= 1,
            .dimensions=dimensions___ReduceMean_axes,
            .memType= QNN_TENSORMEMTYPE_RAW,
            {.clientBuf= { .data=(uint8_t*)__ReduceMean_axes,
                           .dataSize=4}},
            .isDynamicDimensions= nullptr,
            .sparseParams= { QNN_SPARSE_LAYOUT_UNDEFINED,
                             .hybridCoo= {.numSpecifiedElements= 0, .numSparseDimensions= 0}},
  ...
  uint32_t dimensions_output[] = {1, 288, 384, 1};
...
ModelError_t QnnModel_composeGraphs(Qnn_BackendHandle_t backendHandle,
  ...
  /* model/graph for channel_average*/
  QnnModel channel_average;
  const QnnGraph_Config_t** graphConfigs = nullptr;
  VALIDATE(getQnnGraphConfigFromInfo("channel_average", graphsConfigInfo, numGraphsConfigInfo, graphConfigs), err);
  VALIDATE(channel_average.initialize(backendHandle, interface, contextHandle, "channel_average", debug, DO_GRAPH_NODE_VALIDATIONS, graphConfigs), err);
  VALIDATE(addTensor_input(channel_average), err);
  VALIDATE(addNode__ReduceMean(channel_average), err);

  // Add all models to array to get graphsInfo
  QnnModel* models [] = {&channel_average};
